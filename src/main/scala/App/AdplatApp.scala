...
object AdplatApp {
  def main(args: Array[String]): Unit = {
    ...
    //通过SQL处理数据文件
    val result = sqlContext.sql("select adplatformproviderid,count(*) adplatct " +
      "from log group by adplatformproviderid")
    ...
    result.write.mode(SaveMode.Append).jdbc(
      load.getString("jdbc.url"),load.getString("jdbc.tbn5"),props)
