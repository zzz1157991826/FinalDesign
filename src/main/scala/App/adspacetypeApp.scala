...
object adspacetypeApp {
  def main(args: Array[String]): Unit = {
    ...
    //通过SQL处理数据文件
    val result = sqlContext.sql("select adspacetypename,count(*) adspacect " +
      "from log group by adspacetypename")
    ...
    result.write.mode(SaveMode.Append).jdbc(
      load.getString("jdbc.url"),load.getString("jdbc.tbn6"),props)
